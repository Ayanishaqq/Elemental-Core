...
// Continuing from where it stopped...
                ...
                player.addPotionEffect(new PotionEffect(PotionEffectType.DOLPHINS_GRACE, 40, 1, true, false));
                break;
            case "fire":
                ...
                break;
            case "air":
                player.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 40, 2, true, false));
            ...
            case "light":
                ...
                player.addPotionEffect(new PotionEffect(PotionEffectType.ABSORPTION, 40, 1, true, false));
                break;
        }
    }
    ...
    private void useNormalAbility(Player player, String coreName) {
        ...
        switch (coreName.toLowerCase()) {
            case "earth":
                ...
                earthFortress(player);
                setCooldown(player, abilityName, 12);
                break;
    ...
    private void useShiftAbility(Player player, String coreName) {
        ...
        switch (coreName.toLowerCase()) {
            case "earth":
                ...
                earthquake(player);
                setCooldown(player, abilityName, 18);
                break;
    ...
    // Earth Abilities
    private void earthFortress(Player player) {
...
```java
    ...
    // Water Abilities
    private void tidalWave(Player player) {
    ...
    // Fire Abilities
    private void infernoMeteor(Player player) {
    ...
    private void ringOfFire(Player player) {
        Location center = player.getLocation();
    ...
    // Air Abilities
    private void hurricaneLeap(Player player) {
    ...
    // Lightning Abilities
    private void thunderstorm(Player player) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    if (target.isValid()) {
                        ...
                        ((LivingEntity) target).damage(8.0, player);
    ...
    private void lightningDash(Player player) {
        Location start = player.getLocation();
        Vector direction = player.getLocation().getDirection().normalize();
    ...
    // Nature Abilities
    private void naturesEmbrace(Player player) {
        Location center = player.getLocation();
        ...
        for (Entity entity : player.getNearbyEntities(10, 10, 10)) {
            if (entity instanceof Player) {
                ...
                ally.setHealth(Math.min(ally.getHealth() + 16.0, ally.getMaxHealth()));
        ...
        // Heal self
        player.setHealth(Math.min(player.getHealth() + 16.0, player.getMaxHealth()));
    ...
    private void thornfield(Player player) {
        Location center = player.getLocation();
    ...
    // Shadow Abilities
    private void shadowCloneArmy(Player player) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            clone.addPotionEffect(new PotionEffect(PotionEffectType.INCREASE_DAMAGE, 200, 0));
    ...
    private void voidStep(Player player) {
        Location start = player.getLocation();
            ...
            public void run() {
                ...
                for (Entity entity : center.getWorld().getNearbyEntities(center, 10, 10, 10)) {
                    if (entity instanceof Player) {
                        ...
                        // Heal and protect
                        ally.setHealth(Math.min(ally.getHealth() + 1, ally.getMaxHealth()));
