...
public class CoreManager {
    ...
    public ItemStack createCore(CoreType type, int tier) {
        ...
        lore.add("Passive: " + (type.getPassive1() != null ? type.getPassive1().getName() : "") + ", " + (type.getPassive2() != null ? type.getPassive2().getName() : ""));
    ...
    public void giveRandomCore(Player player) {
        ...
        // Play new spinning title animation
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                player.sendTitle(ChatColor.GREEN + "You received:", randomType.getColorHex() + randomType.getName() + " Core!", 10, 70, 20);
            }
    ...
    private void playAnimation(Player player) {
        // New spinning title animation: Cycle through all core names for 7 seconds
        CoreType[] cores = CoreType.values();
        new BukkitRunnable() {
            int time = 0;
            int index = 0;
            @Override
            public void run() {
                ...
                // Cycle through cores (spinning effect by updating title)
                CoreType current = cores[index % cores.length];
                String title = current.getColorHex() + current.getName();
                String subtitle = "Summoning Elemental Core...";
                player.sendTitle(title, subtitle, 0, 20, 0); // Instant update for spinning feel

                index++; // Cycle to next core
                time += 10; // Update every 0.5 seconds (10 ticks)
            }
